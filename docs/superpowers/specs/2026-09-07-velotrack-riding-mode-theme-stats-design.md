...
| E-map | `tools/gen-style.mjs`, `tools/build-assets.sh`, `map/MapController.kt` (`lightMap`, light empty style), `test/.../map/StyleTemplateTest.kt` untouched |
...
- `res/font/interstate.xml`:
  ...
  <font-family xmlns:android="http://schemas.android.com/apk/res/android" xmlns:app="http://schemas.android.com/apk/res-auto">
      <font android:font="@font/interstate_comp" android:fontStyle="normal" android:fontWeight="400"
            app:font="@font/interstate_comp" app:fontStyle="normal" app:fontWeight="400" />
  </font-family>
  ```
...
- `themes.xml`: parent `Theme.Material3.DayNight.NoActionBar`; add `<item name="android:fontFamily">@font/interstate</item>` and `<item name="fontFamily">@font/interstate</item>` and the 15 `textAppearance<Role>` items pointing at the styles above; keep the colour role items (they now resolve to light or dark via the two colors.xml files); `android:windowLightStatusBar` → `true` in `values/themes.xml` and a `values-night/themes.xml` that redefines `Theme.VeloTrack` fully (copy of the light one with `windowLightStatusBar=false`) — simpler: put the whole style in both files, identical except that item.
...
- `gen-style.mjs`: CLI `node gen-style.mjs <outStyle> [--flavor dark|light]` (default dark). Dark keeps the current black patch and name `velotrack-dark`; light uses `namedFlavor("light")` unpatched (name `velotrack-light`) except: keep `crossSourceCollisions(false)`… (that is a runtime option, not style) — no other patches. `vector_layers.json` written in both runs (same content). 
- `build-assets.sh`: after the existing dark generation add `node gen-style.mjs "$ASSETS_DIR/style_light.json" --flavor light` and include `style_light.json` in the final existence check.
- `MapController`: `var lightMap: Boolean = false` (read at `loadStyle` time: template asset `style_light.json` when true, else `style.json`); `EMPTY_STYLE_JSON` becomes a function `emptyStyleJson()` returning a background-only style with `#F2F2F2` when `lightMap` else `#000000`; the reload pass-through uses the same function. Public API otherwise unchanged (`loadStyle(bandFiles, onDone)` signature stays).
