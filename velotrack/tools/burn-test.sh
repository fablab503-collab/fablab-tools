...
[ -n "$DEV" ] || { echo "No device. Start the emulator or plug the phone in."; exit 2; }
echo "device:  $DEV"
echo "package: $PKG"
...
"$ADB" -s "$DEV" shell am start -n "$PKG/com.fablab503.velotrack.ui.DownloadMapActivity" >/dev/null 2>&1
sleep 3
PEAK=0
